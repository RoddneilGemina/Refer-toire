...
class UpdateServiceManager {
    ...
    try {
      ...
      try {
        const ghResp = await fetch(
          `https://raw.githubusercontent.com/refertoire/Refertoire/main/app.json?t=${Date.now()}`,
        ...
        if (ghResp.ok) {
          ...
          if (this.isNewer(currentVersion, currentBuildNumber, ghVersion, ghBuild)) {
            const ghRelease: AppRelease = {
              ...
              apkUrl: 'https://github.com/refertoire/Refertoire/raw/main/Refertoire.apk',
              updateType: 'native_build',
            };
  ...
  public async downloadAndInstallUpdate(
    ...
    const downloadUrl =
      release.apkUrl ||
      'https://github.com/refertoire/Refertoire/raw/main/Refertoire.apk';
